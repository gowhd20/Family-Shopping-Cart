 ...
 */
public class CreateFamily extends AsyncTask<Void, Void, Void> {

    Runnable mR;
    ProgressDialog dialog;
    String res_str = "";
    final String url, familyName, macAddress, userName, googleToken;
    Context context;
    DatabaseHelper database;

    public CreateFamily(Context CONTEXT, final String URL, final String FamilyName, final String MacAddress, final String UserName, final String GoogleToken) {
        context = CONTEXT;
            ...
            public void run() {
                mR = new Runnable() {
                    ...
                    public void run() {
                        ...
                        obj.addProperty("family_name", familyName);
    ...
    public void UpdateLocalDatabaseMembers(){
        ...
        JSONObject objData, obj;
        try{
            obj = new JSONObject(res_str);
            objData = obj.optJSONObject("data");
            FamilyGroupActivity.userID_static = objData.getString("uid");
            FamilyGroupActivity.familyID_static = objData.getString("uuid");
            FamilyGroupActivity.familyName_static = familyName;
            Log.d("main", FamilyGroupActivity.userID_static);

            database.clearFamilyMembers();
            Boolean isInserted = database.insertMemberData(userName, FamilyGroupActivity.userID_static);
            Log.d("main", "" + isInserted);
